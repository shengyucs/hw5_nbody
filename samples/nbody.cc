...
int main(int argc, char** argv) {
    ...
    // Problem 3 (sequential reference implementation)

    double best_cost = std::numeric_limits<double>::infinity();
    int best_device = -1;
    int best_step = -1;

    // 找出所有 gravity device 的 id
    std::vector<int> devices;
    for (int i = 0; i < n; i++) {
        if (type[i] == "device") devices.push_back(i);
    }

    for (int didx = 0; didx < (int)devices.size(); didx++) {
        int dev_id = devices[didx];

        for (int fire_step = 0; fire_step <= param::n_steps; fire_step++) {
            ...
            if (!hit_planet) {
                double cost = param::get_missile_cost(fire_step * param::dt);
                if (cost < best_cost) {
                    best_cost = cost;
                    best_device = dev_id;
                    best_step = fire_step;
    ...
    missile_cost = (best_step >= 0) ? param::get_missile_cost(best_step * param::dt) : -1;
